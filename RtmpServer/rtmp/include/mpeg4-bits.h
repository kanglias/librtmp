...
static void mpeg4_bits_init(struct mpeg4_bits_t* bits, void* data, size_t size)
{
...
static int mpeg4_bits_error(struct mpeg4_bits_t* bits)
{
	//return bits->bits >= bits->size * 8 ? 1 : 0;
	return bits->error;
}

static void mpeg4_bits_aligment(struct mpeg4_bits_t* bits, int n)
{
	bits->bits = (bits->bits + n - 1) / n * n;
}

static size_t mpeg4_bits_remain(struct mpeg4_bits_t* bits)
{
...
static int mpeg4_bits_read(struct mpeg4_bits_t* bits)
{
	uint8_t bit;
...
static uint64_t mpeg4_bits_read_n(struct mpeg4_bits_t* bits, int n)
{
	int m;
	size_t i;
	uint64_t v;
...
static int mpeg4_bits_write(struct mpeg4_bits_t* bits, int v)
{
	assert(bits && bits->data && bits->size > 0);
	if (bits->bits >= bits->size * 8)
	{
...
static int mpeg4_bits_write_n(struct mpeg4_bits_t* bits, uint64_t v, int n)
{
	int m;
	size_t i;
